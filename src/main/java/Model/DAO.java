...
public abstract class DAO {
    ...
    protected ResultSet getResultSet(String query){
        Statement s;        //Usado para instrucoes SQL
        ResultSet rs = null;            //Guarda dados advindos do BD
        try{
            s = (Statement) con.createStatement();      //Permite criar a instrução na conexao estabelecida
            rs = s.executeQuery(query);                 //Executa a instrução query e retorna o resultado para rs
        }
        catch (SQLException e){
            System.err.println("Exception: " + e.getMessage());
        }
    ...
    protected int executeUpdate(PreparedStatement queryStatement) throws SQLException {         //PreparedStatement e uma query mais segura
        int update;
    ...
    protected final boolean createTable() {
        try {
            ...
            stmt = DAO.getConnection().prepareStatement("CREATE TABLE IF NOT EXISTS cliente( \n"
                    ...
                    + "email VARCHAR, \n"
                    + "telefone VARCHAR); \n");
            executeUpdate(stmt);                //stmt.executeUpdate()???
            // Tabela animal:
